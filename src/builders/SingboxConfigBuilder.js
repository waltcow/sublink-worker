...
export class SingboxConfigBuilder extends BaseConfigBuilder {
  ...
  hasOutboundTag(tag) {
    ...
    return (this.config.outbounds || []).some(
      (outbound) => normalize(outbound?.tag) === target,
    );
  }
  ...
  addCountryGroups() {
    ...
    const nodeSelectGroup = this.config.outbounds.find(
      (o) => normalize(o?.tag) === normalize(nodeSelectTag),
    );
  ...
  formatConfig() {
    ...
    rules
      .filter((rule) => !!rule.domain_suffix || !!rule.domain_keyword)
      .map((rule) => {
    ...
    rules
      ...
      .map((rule) => {
        this.config.route.rules.push({
          rule_set: [
            ...(rule.site_rules.length > 0 && rule.site_rules[0] !== ""
              ? rule.site_rules
              : []),
          ],
          protocol: rule.protocol,
    ...
    if (
      ...
      const externalUiDownloadDetour =
        existingClashApi.external_ui_download_detour || defaultDownloadDetour;
