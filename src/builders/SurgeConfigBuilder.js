...
export class SurgeConfigBuilder extends BaseConfigBuilder {
  ...
  addCountryGroups() {
    ...
    const manualProxyNames = proxies
      ...
      if (!existing.has(manualNorm)) {
        this.config["proxy-groups"].push(
          this.createProxyGroup(
            name,
            "select",
            this.sanitizeOptions(manualProxyNames),
          ),
        );
    ...
    const nodeSelectGroupIndex = this.config["proxy-groups"].findIndex(
      (g) => this.getGroupName(g) === this.t("outboundNames.Node Select"),
    );
    if (nodeSelectGroupIndex > -1) {
      ...
      const newGroup = this.createProxyGroup(
        this.t("outboundNames.Node Select"),
        "select",
        newOptions,
      );
  ...
  formatConfig() {
    ...
    if (this.subscriptionUrl) {
      finalConfig.push(
        `#!MANAGED-CONFIG ${this.subscriptionUrl} interval=43200 strict=false`,
      );
    ...
    if (this.config["proxy-groups"]) {
      ...
      const groupStrings = this.config["proxy-groups"]
        .map((group) => {
          if (typeof group === "string") {
            return group;
          } else if (group && typeof group === "object") {
    ...
    rules
      .filter((rule) => !!rule.domain_suffix)
      .map((rule) => {
        rule.domain_suffix.forEach((suffix) => {
          finalConfig.push(
            `DOMAIN-SUFFIX,${suffix},${this.t("outboundNames." + rule.outbound)}`,
          );
        });
      });

    rules
      .filter((rule) => !!rule.domain_keyword)
      .map((rule) => {
        rule.domain_keyword.forEach((keyword) => {
          finalConfig.push(
            `DOMAIN-KEYWORD,${keyword},${this.t("outboundNames." + rule.outbound)}`,
          );
        });
      });
    ...
    rules
      ...
      .map((rule) => {
        rule.site_rules.forEach((site) => {
          finalConfig.push(
            `RULE-SET,${SURGE_SITE_RULE_SET_BASEURL}${site}.conf,${this.t("outboundNames." + rule.outbound)}`,
          );
        });
      });
    ...
    rules
      ...
      .map((rule) => {
        rule.ip_rules.forEach((ip) => {
          finalConfig.push(
            `RULE-SET,${SURGE_IP_RULE_SET_BASEURL}${ip}.txt,${this.t("outboundNames." + rule.outbound)},no-resolve`,
          );
        });
      });

    rules
      .filter((rule) => !!rule.ip_cidr)
      .map((rule) => {
        rule.ip_cidr.forEach((cidr) => {
          finalConfig.push(
            `IP-CIDR,${cidr},${this.t("outboundNames." + rule.outbound)},no-resolve`,
          );
        });
      });
