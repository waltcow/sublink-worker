...
export function addProxyWithDedup(
  collection,
  proxy,
  {
    getName = defaultGetName,
    setName = defaultSetName,
    isSame = defaultIsSame,
  } = {},
) {
  if (!proxy) return;
  if (!Array.isArray(collection)) {
    throw new Error(
      "addProxyWithDedup expects the target collection to be an array",
    );
  }
  ...
  if (
    similarProxies.length > 0 &&
    typeof setName === "function" &&
    targetName
  ) {
    const updated = setName(
      candidate,
      `${targetName} ${similarProxies.length + 1}`,
    );
