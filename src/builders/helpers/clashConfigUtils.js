export function emitClashRules(rules = [], translator) {
  ...
  rules
    .filter(
      (rule) =>
        Array.isArray(rule.domain_suffix) && rule.domain_suffix.length > 0,
    )
    .forEach((rule) => {
      rule.domain_suffix.forEach((suffix) => {
        results.push(
          `DOMAIN-SUFFIX,${suffix},${translator("outboundNames." + rule.outbound)}`,
        );
      });
    });
  ...
  rules
    .filter(
      (rule) =>
        Array.isArray(rule.domain_keyword) && rule.domain_keyword.length > 0,
    )
    .forEach((rule) => {
      rule.domain_keyword.forEach((keyword) => {
        results.push(
          `DOMAIN-KEYWORD,${keyword},${translator("outboundNames." + rule.outbound)}`,
        );
      });
    });
  ...
  rules
    ...
    .forEach((rule) => {
      rule.site_rules.forEach((site) => {
        results.push(
          `RULE-SET,${site},${translator("outboundNames." + rule.outbound)}`,
        );
      });
    });
  ...
  rules
    ...
    .forEach((rule) => {
      rule.ip_rules.forEach((ip) => {
        results.push(
          `RULE-SET,${ip},${translator("outboundNames." + rule.outbound)},no-resolve`,
        );
      });
    });
  ...
  rules
    ...
    .forEach((rule) => {
      rule.ip_cidr.forEach((cidr) => {
        results.push(
          `IP-CIDR,${cidr},${translator("outboundNames." + rule.outbound)},no-resolve`,
        );
      });
    });
...
export function sanitizeClashProxyGroups(config) {
  ...
  const proxyNames = new Set(
    (config.proxies || []).map((p) => normalize(p?.name)).filter(Boolean),
  );
  const groupNames = new Set(
    groups.map((g) => normalize(g?.name)).filter(Boolean),
  );
