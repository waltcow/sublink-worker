...
const QUANX_RULE_TAG_MAP = {
  ...
  "category-ai-!cn": ["OpenAI/OpenAI.list", "Claude/Claude.list"],
...
export class QuanxConfigBuilder extends BaseConfigBuilder {
  ...
  convertProxy(proxy) {
    ...
    const fastOpen = formatFlag(
      proxy.tcp_fast_open ?? proxy.fast_open ?? false,
    );
  ...
  resolvePolicyMembers(outbound, policyName, availableGroupNames = []) {
    ...
    const orderedRegions = REGION_ORDER.filter((region) =>
      availableGroupNames.includes(region),
    );
    ...
    if (policyName === "Apple") {
      const appleRegions = ["Hong Kong", "Japan", "United States"].filter((r) =>
        orderedRegions.includes(r),
      );
    ...
    if (policyName === "AI Suite") {
      const aiRegions = ["United States", "Taiwan", "Japan"].filter((r) =>
        orderedRegions.includes(r),
      );
    ...
    if (
      ...
      const mediaRegions = [
        ...
        "Taiwan",
        "United States",
      ].filter((r) => orderedRegions.includes(r));
      return ["Global Media", ...mediaRegions];
    }

    // Max: Global Media, 地区特定
    if (policyName === "Max") {
      const maxRegions = ["Singapore", "Hong Kong", "Taiwan"].filter((r) =>
        orderedRegions.includes(r),
      );
    ...
    if (policyName === "Crypto") {
      const cryptoRegions = ["Singapore", "Taiwan"].filter((r) =>
        orderedRegions.includes(r),
      );
  ...
  ensurePolicyGroups(rules) {
    const basePolicy = Array.isArray(this.config.policy)
      ? this.config.policy
      : [];
    ...
    // 添加基础策略组（确保这些总是存在）
    ["Outside", "Mainland", "Others", "Global Media", "Asian Media"].forEach(
      (p) => requiredPolicies.add(p),
    );

    const { availableGroups, regionIconMap } =
      this.buildAvailableGroups(iconMap);
    ...
    policyLines.push(
      "# China Media 为中国媒体，Global Media 为国际媒体，Outside 为境外链接，Mainland 为大陆链接，Others 为最终规则",
    );
    ...
    policyOrder.forEach((policyName) => {
      ...
      const outbound = Object.entries(OUTBOUND_POLICY_ALIAS).find(
        ([, alias]) => alias === policyName,
      )?.[0];
      const members = this.resolvePolicyMembers(
        outbound,
        policyName,
        availableNames,
      );
  ...
  buildAvailableGroups(iconMap) {
    ...
    const regionIconMap = new Map([
      ...
      [
        "Other",
        "https://testingcf.jsdelivr.net/gh/Koolson/Qure@master/IconSet/Color/Available.png",
      ],
    ]);
  ...
  buildFilterRemote(rules) {
    ...
    const addEntry = (url, policyName, tag) => {
      ...
      seen.add(key);
      entries.push(
        `${url}, tag=${tag}, force-policy=${policyName}, update-interval=${DEFAULT_UPDATE_INTERVAL}, enabled=true`,
      );
    };
    ...
    rules.forEach((rule) => {
      ...
      normalizeList(rule.site_rules).forEach((site) => {
        ...
        const paths = Array.isArray(rulePath)
          ? rulePath
          : rulePath
            ? [rulePath]
            : [];
