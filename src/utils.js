...
export function tryDecodeSubscriptionLines(
  input,
 ...
 */
export function groupProxiesByKeyword(
  proxies,
  keywordGroups = [],
  { getName } = {},
) {
  ...
  keywordGroups.forEach((group) => {
    if (group.name) {
      grouped[group.name] = {
        ...
        includeDirect:
          group.includeDirect !== undefined ? group.includeDirect : false,
...
export function parseCountryFromNodeName(nodeName) {
  const countryData = {
    ...
    AE: {
      ...
      emoji: "🇦🇪",
      aliases: ["阿联酋", "United Arab Emirates"],
    },
  };
  ...
  if (matches && matches.length > 0) {
    ...
    for (const code in countryData) {
      if (
        countryData[code].aliases.some(
          (alias) => alias.toLowerCase() === matchedAlias.toLowerCase(),
        )
      ) {
