...
function runWranglerCommand(command, ignoreError = false) {
  try {
    return execSync(`npx wrangler ${command}`, {
      encoding: "utf8",
      stdio: "pipe",
    });
  } catch (error) {
...
function checkKvNamespaceExists() {
  console.log(
    `正在检查KV namespace ${SUPPORTED_TITLES.map((title) => `"${title}"`).join("、")} 是否存在...`,
  );
  let output;
  try {
...
function createKvNamespace() {
  ...
  try {
    ...
    if (output.includes("code: 10014") || output.includes("already exists")) {
      console.log(
        `KV namespace "${KV_NAMESPACE_NAME}" 已存在 (虽然未在列表显示)。尝试使用 wrangler.toml 中的现有 ID。`,
      );
      ...
      if (existingId) {
        ...
        console.error(
          "无法自动获取 ID：Namespace 已存在但在 wrangler.toml 中未找到现有 ID。请手动检查 Cloudflare 控制台。",
        );
        process.exit(1);
      }
    }
...
function updateWranglerConfig(kvNamespaceId) {
  ...
  try {
    ...
    const kvConfigRegex =
      /kv_namespaces\s*=\s*\[\s*{\s*binding\s*=\s*"SUBLINK_KV"\s*,\s*id\s*=\s*"([^"]*)"\s*}\s*\]/;
...
function main() {
  ...
  if (!namespace) {
    ...
    console.log(
      `KV namespace "${KV_NAMESPACE_NAME}"创建成功，ID: ${namespace.id}`,
    );
  } else {
    console.log(
      `KV namespace "${KV_NAMESPACE_NAME}"已存在，ID: ${namespace.id}`,
    );
  }
